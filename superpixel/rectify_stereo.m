...
while rectify_num < 5
  ...
  Q = rectify_num;
  metricThreshold = 4000 / Q;     % Leads to more interesting points
  matchThreshold = 20 / Q;         % Leads to more matching point pairs
  ...
  blobs1_surf = detectSURFFeatures(I1, 'MetricThreshold', metricThreshold);
  blobs2_surf = detectSURFFeatures(I2, 'MetricThreshold', metricThreshold);
  blobs1_brisk = detectBRISKFeatures(I1);
  blobs2_brisk = detectBRISKFeatures(I2);
  
  %------------------------------------------------------------------------
  % Step 3. Select Correspondences Between Points Based on SURF Features
  [features1_surf, validBlobs1_surf] = extractFeatures(I1, blobs1_surf);
  [features2_surf, validBlobs2_surf] = extractFeatures(I2, blobs2_surf);
  indexPairs_surf = matchFeatures(features1_surf, features2_surf, 'Metric', 'SAD', ...
    'MatchThreshold', matchThreshold);

  [features1_brisk, validBlobs1_brisk] = extractFeatures(I1, blobs1_brisk);
  [features2_brisk, validBlobs2_brisk] = extractFeatures(I2, blobs2_brisk);
  indexPairs_brisk = matchFeatures(features1_brisk, features2_brisk, 'Metric', 'SAD', ...
    'MatchThreshold', matchThreshold);
  
  % Retrieve locations of matched points for each image
  matchedPoints1 = [validBlobs1_surf(indexPairs_surf(:,1),:).Location;...
                    validBlobs1_brisk(indexPairs_brisk(:,1),:).Location];
  matchedPoints2 = [validBlobs2_surf(indexPairs_surf(:,2),:).Location;...
                    validBlobs2_brisk(indexPairs_brisk(:,2),:).Location];
...
points1 = transformPointsForward(tform1, pts1);
points2 = transformPointsForward(tform2, pts2);
